    ...
    useEffect(() => {
        const getfinalUserPerformance = async () => {
            try {
                ...
                if (quarter === "1") {
                    const hasFqResultZero = response.data.some(item => item.fq_results === 0);
                    if (hasFqResultZero) {
                    ...
                    const iffqExists = response.data.some(item => item.fq_ratee_achievement === '' || item.fq_ratee_achievement === null || item.fq_ratee_achievement === undefined);
                    if (iffqExists) {
                        setIfExists(false);
                    }
                    else {
                    ...
                    const hasMyrResultZero = response.data.some(item => item.myr_results === 0);
                    if (hasMyrResultZero) {
                        setCheckQuarter(false);
                    }
                    else {
                    ...
                    const ifMyrExists = response.data.some(item => item.myr_ratee_achievement === '');
                    if (ifMyrExists) {
                        setIfExists(false);
                    }
                    else {
                    ...
                    const hasTqResultZero = response.data.some(item => item.tq_results === 0);
                    if (hasTqResultZero) {
                        setCheckQuarter(false);
                    }
                    else {
                    ...
                    const ifTqExists = response.data.some(item => item.tq_ratee_achievement === '');
                    if (ifTqExists) {
                        setIfExists(false);
                    }
                    else {
                    ...
                    const hasYeeResultZero = response.data.some(item => item.yee_results === 0);
                    if (hasYeeResultZero) {
                        setCheckQuarter(false);
                    }
                    else {
                    ...
                    const ifYeeExists = response.data.some(item => item.yee_ratee_achievement === '');
                    if (ifYeeExists) {
                        setIfExists(false);
                    }
                    else {
