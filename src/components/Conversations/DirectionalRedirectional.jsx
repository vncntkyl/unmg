...
export default function DirectionalRedirectional({ employee_id, convo_type }) {
  ...
  const handleClick = (convoID) => {
    ...
    navigate("/conversations/directional_redirectional/messages");
  }
  //View Employees Conversations
  const handleViewEmployeeClick = (convoID) => {
    localStorage.setItem("convoID", convoID);
    navigate("/conversations/directional_redirectional/view_employee_messages");
  };
  return loading ? (
    "Loading..."
  ) : (
