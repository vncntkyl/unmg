...
export default function Messages({ convoID, employee_id, replyDetails, containerHeight, replyContainerHeight, fileContainerHeight }) {
  ...
  //       // Do not scroll into view if the user is scrolling up
  //       return;
  //     }

  //     if (convo.length > 0 && !initialRender.current) {
  //       loadMessageRef.current?.scrollIntoView({
  //         block: 'start',
  //         inline: 'nearest',
  //       });
  //       toggleLoading(false);
  ...
  //   if (initialRender.current && convo.length > 0) {
  //     loadMessageRef.current?.scrollIntoView({
  //       block: 'start',
  //       inline: 'nearest',
  //     });
  ...
  const handleGoToMessage = (messageId) => {
    ...
    if (targetElement) {
      targetElement.scrollIntoView({ behavior: "smooth", block: "start", offset: 10, });
    }
  };
