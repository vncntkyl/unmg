...
export default function Coaching({ employee_id, convo_type }) {
  ...
  const handleClick = (convoID) => {
    ...
    navigate("/conversations/coaching/messages");
  }
  const coachingType = [
    "Corrective",
    "Developmental",
  ...
  const handleViewEmployeeClick = (convoID) => {
    localStorage.setItem("convoID", convoID);
    navigate("/conversations/coaching/view_employee_messages");
  };
  return loading ? (
    "Loading..."
  ) : (
    <>
      <div className="w-full mt-4">
        <div className="h-[2.5rem] border-t border-x bg-default border-default-dark rounded-t-md p-2 flex items-center justify-between">
        ...
        <div className="w-full h-[95%] border-x border-b border-default-dark rounded-b-md overflow-scroll">
          {viewtype && viewtype == "1" ? (
            ...
            {coaching.length == 0 ? (
              ...
              <>
                {coaching && 
                coaching.filter(item => parseInt(coach) == item.coaching_type).length > 0 ? (
