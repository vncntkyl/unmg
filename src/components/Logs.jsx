    ...
    <div>
      ...
      {data && data.length > 0 ? (
        <>
          ...
          {/* {data
            .reduce((headerDate, headerIndex) => {
              return (data.indexOf(headerDate) === headerIndex);
            })
            .map((headerDate, key) => (
              <Accordion key={headerDate.ID}>
                <Accordion.Panel>
                  {data
                    .filter(
                      (contentDate, contentIndex) =>
                        headerDate.creation_date === contentDate.creation_date
                    )
                    .map((log, index) => (
                      <>
                        <Accordion.Title>
                          {format(new Date(log.creation_date), "MMMM dd, yyyy")}
                        </Accordion.Title>
                        <Accordion.Content>
